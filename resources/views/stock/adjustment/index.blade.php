                        ...
                        <tbody>
                            @foreach ($adjustments as $key => $item)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>{{ $item->refID }}</td>
                                    <td>{{ $item->product->code }} |{{ $item->product->color }} | {{ $item->product->desc }} | {{ $item->product->cat }}</td>
                                    <td>{{ $item->warehouse->name }}</td>
                                    <td>{{ date('d M Y', strtotime($item->date)) }}</td>
                                    <td>{{ number_format($item->qty) }} {{$item->product->cat == "Kaleen" ? "Nos" : "SQM"}}</td>
                                    <td>
                        ...
                        <div class="form-group mt-2">
                            ...
                            <select name="productID" id="product" required class="selectize">
                                ...
                                @foreach ($products as $product)
                                    <option value="{{ $product->id }}">{{ $product->code }} | {{ $product->color }} |{{ $product->desc }} | {{ $product->cat }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group mt-2">
                            <label for="warehouseID">Warehouse</label>
                            <select name="warehouseID" id="warehouseID" required class="selectize">
                                <option value=""></option>
